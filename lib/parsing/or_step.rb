...
module PARSE_OR_STEP
  ...
  def parse_route_from_or_step( route_id )
    ...
    if @routes_to_steps[@routes[route_id][:route].from_step_id].size != 2
      ...
      logger.error "OR step with name #{@routes[route_id][:route].source_step_name} has #{@routes_to_steps[@routes[route_id][:route].from_step_id].size} inbound routes."
    ...
    # The appearance of inbound routes in first or second place has no meaning beyond the order they are delivered from the data store.
      ...
      if @routes[first_inbound_route_id][:parsed] == true and @routes[second_inbound_route_id][:parsed] == true
        ...
        if @routes[first_inbound_route_id][:status] == 'TRUE' or @routes[second_inbound_route_id][:status] == 'TRUE'
        ...
        elsif @routes[first_inbound_route_id][:status] == 'UNTRAVERSABLE' or @routes[second_inbound_route_id][:status] == 'UNTRAVERSABLE'
