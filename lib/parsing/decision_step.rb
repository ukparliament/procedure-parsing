...
module PARSE_DECISION_STEP
  ...
  def parse_route_from_decision_step( route_id )
    ...
    if @routes_to_steps[@routes[route_id][:route].from_step_id].size != 1
      ...
      logger.error "Decision step with name #{@routes[route_id][:route].source_step_name} has #{@routes_to_steps[@routes[route_id][:route].from_step_id].size} inbound routes."
    ...
    # Otherwise, if the decision step does have one inbound route ...
      ...
      if @routes[inbound_route_id][:parsed] == true
        ...
        if @routes[inbound_route_id][:status] == 'TRUE'
